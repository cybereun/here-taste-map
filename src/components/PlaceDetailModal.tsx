...
export const PlaceDetailModal: React.FC<PlaceDetailModalProps> = ({ place, onClose, isOverseas }) => {
  ...
  // Reset states when place changes or modal opens
  useEffect(() => {
    setLightboxIdx(null);
    setImageErrorMap({});
    setCopied(false);
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-end justify-center bg-black/60">
      ...
      <div className="relative w-full bg-white rounded-t-3xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col z-10">
        {/* Header Bar */}
        ...
        {images.length > 0 && (
          <div className="px-3 py-2 bg-gray-100/90 border-b border-gray-200 shrink-0">
            ...
            <div className="flex flex-row flex-nowrap gap-2 overflow-x-auto no-scrollbar py-0.5 w-full">
              {images.map((img, idx) => (
                <div
                  ...
                  className="relative w-16 h-16 rounded-xl overflow-hidden shrink-0 border-2 border-white shadow-xs hover:border-orange-500 active:scale-95 transition-all cursor-pointer bg-gray-200 flex items-center justify-center"
                >
                  {imageErrorMap[idx] ? (
      ...
      {/* 3. Full-Screen Photo Lightbox (오버레이 잔상 방지 및 안정적인 렌더링) */}
      {lightboxIdx !== null && (
        <div
          onClick={() => setLightboxIdx(null)}
          className="fixed inset-0 z-[99999] bg-black flex flex-col justify-between p-3 select-none"
        >
          {/* Top bar */}
          <div className="flex items-center justify-between text-white py-2 px-3 z-20">
